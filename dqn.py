...
#policy_net.load_state_dict(torch.load('qNet.pkl'))
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        reward = torch.tensor([9.8 * (math.sin(3 * next_state[0]) + 1) + 0.5 * next_state[1] * next_state[1]], device=device)
        ...
        if done:
            ...
            break
    tracex.append(i_episode)
    tracey.append(score / step)
    # Update the target network
    if i_episode % TARGET_UPDATE == 0:
        target_net.load_state_dict(policy_net.state_dict())
    # Save model
    # torch.save(policy_net.state_dict(), 'qNet.pkl')

print('Complete')
plt.plot(tracex, tracey)
plt.xlabel('Episodes')
plt.ylabel('Average Reward throughout an episode')
